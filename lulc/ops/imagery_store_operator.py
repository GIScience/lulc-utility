...
class EarthEngineOperator(ImageryStore):
    ...
    def __init__(self, service_account: str, key_file_location: Path,
                 s1_bands: List[str],
                 s2_bands: List[str],
                 dem_bands: List[str],
                 cache_dir: Path,
    ...
    def imagery(self, area_coords: Tuple, start_date: str, end_date: str,
        ...
        try:
            s1 = read_raster(s1_file) if s1_file.exists() else self.__download_s1_data(s1_file, area_of_interest, start_date, end_date, resolution)
            s2 = read_raster(s2_file) if s2_file.exists() else self.__download_cloudless_s2_data(s2_file, area_of_interest, start_date, end_date, resolution)
            dem = read_raster(dem_file) if dem_file.exists() else self.__download_dem(dem_file, area_of_interest, resolution)
            ...
            log.exception('Data download not possible')
            raise OperatorInteractionException('Earth Engine operator interaction not possible. Please contact platform administrator.')
    ...
    def __download_cloudless_s2_data(self, output_file: Path, area_of_interest: ee.Geometry.BBox, start_date: str, end_date: str, resolution: int):
        ...
        def add_shadow_bands(img):
            ...
            dark_pixels = img.select('B8').lt(self.nir_dark_regions_threshold * self.sr_band_scale).multiply(not_water).rename('dark_pixels')
            shadow_azimuth = ee.Number(90).subtract(ee.Number(img.get('MEAN_SOLAR_AZIMUTH_ANGLE')))
            cloud_projection = (img.select('clouds').directionalDistanceTransform(shadow_azimuth, self.cloud_projection_distance * 10)
        ...
        def add_cld_shdw_mask(img):
            ...
            is_cloud_shadow = (is_cloud_shadow.focalMin(2).focalMax(self.cloud_dilation_buffer * 2 / 20)
        ...
        ee_image = (s2_collection.map(add_cld_shdw_mask)
    ...
    @staticmethod
    def __ee_image_to_numpy(output_file: Path, ee_image: ee.Image, area_of_interest: ee.Geometry.BBox, resolution: int, bands: List[str]):
        ...
        with ThreadPoolExecutor() as pool:
            ...
            profile.update(dtype=rio.int16,
                           count=len(bands),
                           compress='lzw')
...
def resolve_imagery_store(cfg: DictConfig, cache_dir: Path) -> ImageryStore:
    if cfg.operator == 'sentinel_hub':
        return SentinelHubOperator(cfg.api_id, cfg.api_secret, Path(cfg.evalscript_dir), cfg.evalscript_name, cache_dir=cache_dir / 'imagery')
    elif cfg.operator == 'earth_engine':
        return EarthEngineOperator(cfg.service_account, cfg.key_file_location, cfg.s1_bands, cfg.s2_bands, cfg.dem_bands, cache_dir=cache_dir / 'imagery')
    else:
